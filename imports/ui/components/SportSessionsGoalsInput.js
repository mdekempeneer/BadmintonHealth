...
class FootstepGoalsInput extends TrackerReact(Component) {
    ...
    clickAdd(event) {
        if (this.state.sessions === '') {
            Bert.alert('Gelieve een aantal sessies in te vullen.', 'danger', 'fixed-top');
        } else {
            var number = this.state.sessions.replace(',','.');
            ...
            if (check1 || check2) {
                ...
                Meteor.call('personalGoal','sportsessions', number, () => {
                    levelChecker();
                    badgeChecker();
    ...
    render() {
        ...
        var partial = PersonalGoalData.find({ user: Meteor.userId(), type: 'sportsessions', date: CHECK_DATE });

        if (partial.count() >= 1) {
            return (
                <div className='rate-button'>
                    <T>goals.this_week_goal</T> {partial.fetch()[0].amount} <T>goals.sessions</T>
                </div>
            )
        } else {
            return (
                <div className='rate-button'>
                    ...
                    >
                        <Input
                            ...
                            onChange={this.handleChange('sessions').bind(this)}
                            endAdornment={<InputAdornment position='end'>{i18n.__('goals.sessions')}</InputAdornment>}
                        />
                    </FormControl>
